 ...
 * ──────────────────────────────
 * Pure UI — uses hooks for all data fetching, renders sessions table
 * and event journey timeline.
 */
...
function formatDuration(first, last) {
  const totalSecs = Math.floor((new Date(last) - new Date(first)) / 1000);
  if (totalSecs < 60) return `${totalSecs}s`;
  const mins = Math.floor(totalSecs / 60);
  const remainingSecs = totalSecs - mins * 60;
...
function SessionRow({ session, isSelected, onClick }) {
  return (
    ...
    >
      ...
      <td><span style={{ fontWeight: 700, fontSize: '1.1rem' }}>{session.total_events}</span></td>
      <td><span className="badge green">👁 {session.page_view_count}</span></td>
      <td><span className="badge orange">🖱 {session.click_count}</span></td>
      ...
      <td style={{ fontSize: '0.8rem', color: 'var(--text3)' }}>{session.pages_visited}</td>
      <td><span style={{ color: 'var(--text3)', fontSize: '0.85rem' }}>›</span></td>
    </tr>
  );
}

function EventTimeline({ sessionId, onClose }) {
  // Logic lives in the hook — component is pure rendering
...
export default function SessionsView() {
  // All data logic is in the hook — this component only handles UI state (selected row)
  ...
  return (
    <div className="fade-in">
      ...
      <div className="section-header">
        ...
        <button className="refresh-btn" onClick={refetch} id="refresh-sessions-btn">
          ↻ Refresh
        </button>
      </div>
