...
// Draw heatmap dots on canvas using a simple radial gradient approach
function drawHeatmap(canvas, clicks, width, height) {
  ...
  // Dark background representing the webpage
  ...
  scale.forEach(({ x, y }, i) => {
    const ci = Math.min(Math.floor(x / CELL), cols - 1);
    const ri = Math.min(Math.floor(y / CELL), rows - 1);
    const d = density[ri * cols + ci] / maxDensity; // 0–1
...
export default function HeatmapView() {
  ...
  // Redraw canvas whenever click data changes
